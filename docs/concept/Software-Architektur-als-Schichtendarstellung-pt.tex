...
depend\^encias observadas entre classes, distingue dois tipos de ciclos~--
ciclos de classes e ciclos de pacotes~--, torna ambos vis\'iveis como achados
ativados separadamente e verifica a representa\c{c}\~ao final de forma
...
\section{Dois Tipos de Ciclos}

S202 distingue dois tipos fundamentalmente diferentes de ciclos e torna ambos
vis\'iveis como overlays ativados separadamente:

\begin{description}
  \item[Ciclos de classes (vermelho)] Componentes fortemente conexas no n\'ivel
        de classes. Duas ou mais classes que dependem uma da outra direta ou
        transitivamente formam tal ciclo. Ciclos de classes revelam acoplamento
        no n\'ivel de implementa\c{c}\~ao que impede uma ordem linear estrita de
        classes individuais. Eles s\~ao detectados pelo algoritmo de Tarjan sobre
        o grafo de depend\^encias de classes.
  \item[Ciclos de pacotes (laranja)] Tangles no n\'ivel de pacotes, detectados
        sobre o grafo agregado direto de pacotes (sem a propaga\c{c}\~ao de
        ancestrais descrita adiante). Um ciclo de pacotes significa que dois
        ou mais pacotes dependem uns dos outros como grupo, independentemente de
        quais classes concretas s\~ao respons\'aveis. Ciclos de pacotes revelam
        acoplamento no n\'ivel arquitetural: a pr\'opria estrutura de pacotes tem
        depend\^encias circulares.
\end{description}

A distin\c{c}\~ao importa porque os dois tipos respondem a perguntas diferentes:

\begin{itemize}
  \item Um \emph{ciclo de classes} mostra quais classes concretas precisam ser
        desembaraladas. Dissolv\^e-lo frequentemente exige mover um m\'etodo,
        extrair uma interface ou inverter uma depend\^encia entre duas classes
        espec\'ificas.
  \item Um \emph{ciclo de pacotes} mostra onde os pr\'oprios limites de pacotes
        est\~ao quebrados. Resolv\^e-lo pode exigir uma reestrutura\c{c}\~ao
        maior~-- como mesclar dois pacotes, introduzir um novo, ou redefinir
        inteiramente a divis\~ao de m\'odulos.
\end{itemize}

Um ciclo de pacotes pode existir mesmo quando nenhum ciclo de classes individual
\'e vis\'ivel~-- por exemplo, quando dois pacotes se chamam mutuamente por meio
de classes diferentes que s\~ao individualmente ac\'iclicas. Por outro lado, um
ciclo de classes inteiramente contido em um \'unico pacote n\~ao implica
necessariamente um ciclo de pacotes.

Como os dois overlays respondem a perguntas diferentes, S202 os mant\'em como
chaves independentes. Ativar ambos ao mesmo tempo \'e poss\'ivel e \'util ao
buscar a causa raiz de um problema arquitetural, mas cada overlay tamb\'em \'e
significativo por si s\'o: o overlay de ciclos de pacotes revela a forma
estrutural do problema, enquanto o overlay de ciclos de classes revela as
arestas concretas que o causam.
...
recebe peso $n$. Se uma classe tem apenas depend\^encias sem contagem de
chamadas para um pacote de destino, ela contribui com um peso de fallback
de~1 para essa aresta de pacote (por classe e pacote de destino, n\~ao por
depend\^encia).
...
A propaga\c{c}\~ao de ancestrais pode, por\'em, criar ciclos que n\~ao existem
no grafo direto de pacotes. Ciclos de pacotes (tangles) s\~ao, portanto,
detectados sobre o grafo direto, n\~ao propagado; a quebra de ciclos e a
atribui\c{c}\~ao de n\'iveis, em contrapartida, operam sobre o grafo
propagado.

Um exemplo num\'erico concreto: as depend\^encias de classe
\texttt{com.a.A}$\to$\texttt{net.b.B} e \texttt{com.a.B}$\to$\texttt{net.b.B}
geram as seguintes arestas no grafo de pacotes:
...
Apenas o lado da origem \'e propagado pela cadeia de ancestrais; o lado do
destino permanece no pacote pai direto da classe alvo. \texttt{net.b} n\~ao
\'e propagado adiante para \texttt{net}. Propagar tamb\'em o lado do destino
geraria um produto cartesiano de ancestrais de origem e destino para cada
depend\^encia de classe: o n\'umero de arestas cresce quadraticamente com a
profundidade dos pacotes, sem ganho de informa\c{c}\~ao para o c\'alculo da
ordem.
...
Os dois tipos de corte s\~ao aplicados iterativamente: ap\'os cada passagem,
as SCCs s\~ao recalculadas e, para SCCs residuais, os ranks s\~ao recalculados
sobre seus membros. O procedimento termina quando n\~ao resta nenhuma SCC com
...
Essa regra~-- conhecida como nivelamento de \emph{caminho mais longo} ou
\emph{ASAP}~-- ancora cada pacote o mais baixo poss\'ivel, imediatamente acima
de sua depend\^encia mais profunda. Ela coincide com a invariante que o
Structure101 documenta para seus Levelized Structure Maps: cada elemento depende
de pelo menos um elemento no n\'ivel imediatamente abaixo. A alternativa de
puxar cada pacote rente \emph{abaixo de seu consumidor} (ALAP) violaria essa
invariante, pois poderia surgir uma lacuna entre um pacote e sua depend\^encia
mais pr\'oxima.

Uma consequ\^encia \'e aceita deliberadamente: pacotes com o mesmo papel, mas
com cadeias de depend\^encia de profundidade diferente, acabam em n\'iveis
diferentes. Se, por exemplo, v\'arios adaptadores ficam acima da mesma camada de
aplica\c{c}\~ao, mas um deles fica adicionalmente acima de um pacote auxiliar
compartilhado, esse \'e colocado um n\'ivel acima dos demais~-- o nivelamento
mede profundidade de depend\^encia, n\~ao papel arquitetural.
...
arquitetural. O corte \'e aplicado iterativamente at\'e que nenhuma aresta
dentro de uma SCC corra contra a hip\'otese de pacotes; SCCs que restam depois
disso s\~ao tratadas pelo Caso~B.
...
\begin{description}
  ...
  \item[Viola\c{c}\~ao] Uma depend\^encia concreta que, depois da ordem
        calculada, corre para cima. O que conta aqui \'e a ordem
        \emph{aninhada}, n\~ao o n\'ivel global plano: as cadeias de
        \emph{localLevel} dos dois extremos s\~ao comparadas a partir do
        cont\^einer comum mais externo para baixo, e a dire\c{c}\~ao \'e decidida
        pelo primeiro cont\^einer em que diferem~-- ou seja, o \emph{localLevel}
        do n\'ivel-alvo apropriado (chamado \emph{visual rank} no c\'odigo). Uma
        aresta pode correr para baixo globalmente (pelo \emph{architectureLevel})
        e ainda assim apontar para cima entre dois pacotes irm\~aos~-- e nesse
        caso \'e uma viola\c{c}\~ao. Ela pode vir de uma SCC, mas n\~ao precisa.
...
\begin{enumerate}
  \item \textbf{Grafo de classes:} Para depend\^encias normais entre classes
        ...
        exemplo como back edge ou como aresta dentro de uma SCC de classes.
        Ciclos detectados no n\'ivel de classes correspondem ao overlay vermelho
        na interface.
  \item \textbf{Grafo de pacotes:} A ordem de pacotes calculada precisa ser
        ...
        verifica\c{c}\~ao. Ciclos detectados no n\'ivel de pacotes correspondem
        ao overlay laranja na interface.
...
\begin{enumerate}
  \item \textbf{Construir o grafo de classes:} A partir do bytecode \'e criado
        ...
        dom\'inio da an\'alise. Apenas arestas que carregam pelo menos uma
        rela\c{c}\~ao do tipo \texttt{extends}, \texttt{implements},
        \texttt{imports}, \texttt{instantiates} ou \texttt{calls} entram no
        c\'alculo da ordem; refer\^encias puras de tipo (tipos de campos,
        tipos de par\^ametros e retorno, casts) permanecem no modelo bruto,
        mas n\~ao s\~ao consideradas.
  ...
  \item \textbf{Calcular a ordem de pacotes:} A partir das entradas e sa\'idas
        ...
        cortadas; SCCs residuais s\~ao tratadas novamente com ranks
        recalculados, at\'e que o grafo de pacotes seja ac\'iclico. Se todos os
        ranks dentro de uma SCC s\~ao iguais, todas as arestas internas s\~ao
        removidas; os n\'iveis surgem ent\~ao de depend\^encias fora da antiga
        SCC.
  ...
  \item \textbf{Atribuir posi\c{c}\~oes locais de exibi\c{c}\~ao:} Ap\'os a
        ...
        produz um \emph{localLevel} por filho. O \emph{localLevel} governa o
        posicionamento visual dentro da caixa pai e \'e tamb\'em a base da
        detec\c{c}\~ao vis\'ivel de viola\c{c}\~oes (o \emph{visual rank} sobre a
        cadeia de n\'iveis aninhada descrita acima). O \emph{architectureLevel}
        \'e o n\'ivel sem\^antico global; ele aciona o layout e o invariante
        global de consist\^encia~-- uma condi\c{c}\~ao necess\'aria, mas n\~ao
        suficiente, pois uma compara\c{c}\~ao global plana n\~ao detecta arestas
        aninhadas que sobem entre cont\^eineres irm\~aos.
...
\begin{lstlisting}
Bytecode -> grafo de classes -> SCCs de classes (overlay vermelho)
         -> grafo de pacotes -> SCCs / tangles de pacotes (overlay laranja)
         -> architectureLevel de pacotes
...
Uma decis\~ao de projeto fundamental \'e a separa\c{c}\~ao expl\'icita de dois
tipos de ciclos. Ciclos de classes e ciclos de pacotes s\~ao detectados
independentemente, armazenados como achados distintos e expostos por meio de
chaves separadas na interface. Ciclos de classes (vermelho) mostram quais classes
concretas formam cadeias c\'iclicas de depend\^encia; ciclos de pacotes (laranja)
mostram quais pacotes s\~ao mutuamente dependentes no n\'ivel arquitetural. Como
ambas as vis\~oes est\~ao dispon\'iveis simultaneamente, a ferramenta pode
responder a duas perguntas diferentes ao mesmo tempo: \emph{onde} na estrutura
de pacotes est\'a o acoplamento circular, e \emph{quais depend\^encias concretas
entre classes} o causam.
