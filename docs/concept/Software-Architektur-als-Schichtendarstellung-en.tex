...
dependencies, distinguishes two kinds of cycles — class-level cycles and
package-level cycles — makes both visible as separately toggled findings, and
...
\section{Two Kinds of Cycles}

S202 distinguishes two fundamentally different kinds of cycles and makes both
visible as separately toggled overlays:

\begin{description}
  \item[Class cycles (red)] Strongly connected components at the class level.
        Two or more classes that depend on each other directly or transitively
        form such a cycle. Class cycles reveal implementation-level coupling
        that prevents a strict linear order of individual classes. They are
        detected by Tarjan's algorithm on the class dependency graph.
  \item[Package cycles (orange)] Tangles at the package level, detected on the
        direct aggregated package graph (without the ancestor propagation
        described later). A package cycle means that two or more
        packages depend on each other as a group, regardless of which concrete
        classes inside them are responsible. Package cycles reveal
        architectural-level coupling: the package structure itself has circular
        dependencies.
\end{description}
...
\begin{itemize}
  \item A \emph{class cycle} shows where individual classes need to be
        untangled. Breaking it often requires moving a method, extracting an
        interface, or inverting a dependency between two specific classes.
  \item A \emph{package cycle} shows where package boundaries themselves are
        broken. Resolving it may require a larger restructuring — merging two
        packages, introducing a new one, or redefining the module split
        entirely.
\end{itemize}

A package cycle can exist even when no individual class cycle is visible —
for example, when two packages call each other through different classes that
are each individually acyclic. Conversely, a class cycle contained entirely
within a single package does not necessarily imply a package cycle.

Because the two overlays answer different questions, S202 keeps them as
independent switches. Activating both at the same time is possible and useful
when looking for the root cause of an architectural problem, but each overlay
is also meaningful on its own: the package-cycle overlay reveals the
structural shape of the problem, while the class-cycle overlay reveals the
concrete edges that cause it.
...
receives weight $n$. If a class has only dependencies without method-call data
into a target package, it contributes a fallback weight of~1 to that package
edge (per class and target package, not per dependency).
...
Ancestor propagation can, however, create cycles that do not exist in the
direct package graph. Package cycles (tangles) are therefore detected on the
direct, non-propagated package graph; cycle-breaking and level assignment, by
contrast, operate on the propagated graph.

A concrete numerical example: the class dependencies
\texttt{com.a.A}$\to$\texttt{net.b.B} and \texttt{com.a.B}$\to$\texttt{net.b.B}
produce the following edges in the package graph:
...
Only the source side is propagated up the ancestor chain; the target side stays
at the direct parent package of the target class. \texttt{net.b} is not further
propagated to \texttt{net}. Propagating the target side as well would produce a
cross-product of source and target ancestors for every class dependency: the
number of edges grows quadratically with package depth, without any information
gain for the ordering computation.
...
Both kinds of cuts are applied iteratively: after each pass, the SCCs are
recomputed, and for any remaining residual SCCs the ranks are recalculated over
their members. The procedure ends when no SCC with at least two members
remains.
...
This rule — known as \emph{longest-path} or \emph{ASAP} levelization — anchors
every package as low as possible, immediately above its deepest dependency. It
matches the invariant that Structure101 documents for its Levelized Structure
Maps: every item depends on at least one item on the level immediately below it.
The alternative of instead pulling each package flush \emph{beneath its
consumer} (ALAP) would violate this invariant, since a gap could open up between
a package and its nearest dependency.

One consequence is accepted deliberately: packages with the same role but
dependency chains of different depth end up on different levels. If, say,
several adapters sit above the same application layer but one of them
additionally sits above a shared helper package, that one is placed one level
higher than the rest — levelization measures dependency depth, not
architectural role.
...
concrete dependency that violates the package order. The cut is applied
iteratively until no edge inside an SCC runs against the package hypothesis;
SCCs that remain afterwards are handled by Case~B.
...
\begin{description}
  ...
  \item[Violation] A concrete dependency that runs upward after the computed
        order has been applied. What counts here is the \emph{nested} order, not
        the flat global level: the \emph{localLevel} chains of both endpoints are
        compared from the outermost shared container downward, and the direction
        is decided by the first container in which they differ~-- i.e. the
        \emph{localLevel} of the appropriate target level (called \emph{visual
        rank} in the code). An edge can run downward globally (by
        \emph{architectureLevel}) yet still point upward between two sibling
        packages~-- in which case it is a violation. It may come from an SCC, but
        it does not have to. Violations are therefore a superset of back edges: a
        back edge can
...
\begin{enumerate}
  \item \textbf{Class graph:} For normal class dependencies, the following
        ...
        or as an edge inside a class SCC. Class-level cycles detected here
        correspond to the red overlay in the UI.
  \item \textbf{Package graph:} The computed package ordering must be
        ...
        and are exempt from this check. Package-level cycles detected here
        correspond to the orange overlay in the UI.
...
\begin{enumerate}
  \item \textbf{Build the class graph:} A directed class graph is created from
        ...
        Only edges that carry at least one relationship of kind
        \texttt{extends}, \texttt{implements}, \texttt{imports},
        \texttt{instantiates}, or \texttt{calls} enter the ordering
        computation; pure type references (field types, parameter and return
        types, casts) remain in the raw model but are not considered.
  ...
  \item \textbf{Compute package ordering:} The rank measure $rank(P)$ is
        ...
        which $rank(P) < rank(Q)$ holds are classified as back edges and cut;
        residual SCCs are processed again with recomputed ranks until the
        package graph is acyclic. If all ranks within an SCC are equal, all
        internal edges are removed; levels then arise from dependencies outside
        the former SCC.
  ...
  \item \textbf{Assign local display positions:} After class levels are
        ...
        \emph{localLevel} per child. \emph{localLevel} governs visual positioning
        inside the parent box and is also the basis of the visible violation
        detection (the \emph{visual rank} over the nested level chain described
        above). \emph{architectureLevel} is the global semantic level; it drives
        the layout and the global consistency invariant~-- a necessary but not
        sufficient condition, since a flat global comparison does not detect
        nested upward edges between sibling containers.
...
A key design decision is the explicit separation of two kinds of cycles.
Class cycles and package cycles are detected independently, stored as distinct
findings, and exposed through separate toggle switches in the UI. Class cycles
(red) show which concrete classes form cyclic dependency chains;
package cycles (orange) show which packages are mutually dependent at the
architectural level. Because both views are available simultaneously, the tool
can answer two different questions at once: \emph{where} in the package
structure is the circular coupling, and \emph{which concrete class
dependencies} cause it.
