...
the same rank-score algorithm within each parent container independently. It
builds a sibling-only weighted graph of the direct children and derives a
\emph{localLevel} for every element. The local level controls visual
...
\begin{enumerate}
  ...
  \item \textbf{Assign local display positions:} After class levels are
        ...
        children is built; the same rank-score algorithm breaks any cycles;
        longest-path yields a \emph{localLevel} per child. \emph{localLevel}
