...
\begin{enumerate}
  ...
  \item Die sichtbare UI-Ordnung verwendet dieselbe Levelordnung wie das
        ...
        sichtbare Level-Tupel entlang der Paketkette.
\end{enumerate}

Die aktuelle Implementierung zerlegt diese Nachbedingungen in vier konkrete
Checks:

\begin{description}
  \item[R1] Pr\"uft Nachbedingung~1 f\"ur Klassenabh\"angigkeiten: Jede
        nicht als Backedge markierte Kante $A \to B$ muss
        $level(A) > level(B)$ erf\"ullen.
  \item[R2] Pr\"uft Nachbedingung~1 f\"ur Pakete in derselben SCC: Peers
        ohne dominante Abh\"angigkeitsrichtung erhalten denselben Level.
  \item[R3] Pr\"uft Nachbedingung~1 f\"ur Paketabh\"angigkeiten: F\"ur jede
        gewichtete Paketkante mit $w(P,Q) > w(Q,P)$ muss
        $level(P) > level(Q)$ gelten.
  \item[R4] Pr\"uft Nachbedingung~2: Jede Abh\"angigkeit tr\"agt eine
        Klassifikation (NORMAL, VIOLATION, INTRA\_SCC). Kanten ohne
        Klassifikation sind ein Implementierungsfehler.
\end{description}

Nachbedingung~3 (Level-Tupel-Vergleich bei verschachtelten UI-Elementen)
