...
\BlankLine
\textbf{Step~5a:} Lift parent package levels --- single bottom-up pass:\;
\Indp sort packages deepest-first; $\text{parent.level} \gets \max(\text{child.level})$\;\Indm
\BlankLine
\textbf{Step~5b:} Order packages by cross-package dependencies --- single topo-sort pass:\;
\Indp DFS post-order on cross-package DAG; $\text{pkg.level} \gets \max(\text{dep.level})+1$\;\Indm
\BlankLine
\textbf{Step~5c:} Equalize package SCCs (R2) --- single pass:\;
\Indp Tarjan on cross-package graph; lift SCC members to $\max(\text{level})$\;\Indm
\BlankLine
...
Steps~5a--5c replace what an earlier version of the pipeline expressed as a
fixpoint loop. The loop appeared natural because package-SCC equalization can
lift a package level, which can lift its parent, which can seem to require
another equalization pass. However, after the SCC condensation in Step~3 the
class dependency graph is a DAG by construction, and longest-path computation on
a DAG converges in \emph{one} topological-order pass --- Bellman-Ford relaxation
on a DAG needs only $O(V+E)$, not $O(V \cdot E)$ iterations. The same argument
applies to the cross-package ordering in Step~5b: processing packages in
depth-first post-order (dependencies before dependents) makes every level
assignment final in a single traversal. The original loop with a 20-iteration
guard was a symptom of processing nodes in an arbitrary order; sorting by
topological position eliminates the need for re-processing entirely.
...
implementation steps. Phase 1 is an ordered pipeline of single topological-sort passes.
Phase 2 is an independent local computation.
