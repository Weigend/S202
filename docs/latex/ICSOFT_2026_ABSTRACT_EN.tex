...
correctness should be the tool's responsibility, not the reader's---the
implementation shows how hard that actually is.
\par\smallskip
...
identical findings on shared inputs. To our knowledge, no previous layered
...
\begin{enumerate}
  ...
  \item \textbf{Package-order constraint (R3):} In the weighted inter-package
        dependency graph, the package with the dominant call volume must be placed
        at a strictly higher level than the package it calls into.
...
The separation of class levels and package levels is not a design choice---no
correct level-computation pipeline can derive both coordinate systems from a
single graph in a single pass.
...
$\rightarrow$ SCCBreaker: identify heuristic back-edges in large SCCs ($|S| \geq 3$)\;
...
\Indp
  Tarjan on current graph $\rightarrow$ find SCCs with $|S|\ge 2$\;
...
When we deployed the invariant checker against large real-world code bases, it
guided refinement of both the pipeline and the rules themselves.
Against Minecraft Forge (2\,895 classes, 220 packages), the checker initially
reported two R2 violations. Investigation showed these were \emph{false positives}:
R2 filtered class-level heuristic back-edges but not the package-level back-edges
that the package SCC-breaker had already removed. R3 already applied the correct
filter; once R2 was aligned, the Minecraft findings disappeared.
Figure~\ref{fig:invariant-r2} shows the original R2 output before this refinement.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{\detokenize{figures/Invariant Finding R2.png}}
  \caption{R2 checker output on Minecraft Forge before the package back-edge filter
  was added. The report was a false positive: the packages were correctly broken out
  of their SCC by the package-level heuristic---R2 had not yet been taught to
  respect that cut. False positives of this kind forced the rule to become more precise.}
  \label{fig:invariant-r2}
\end{figure}

Against \emph{software-ekg-7}, in contrast, R2 surfaced a genuine pipeline bug:
a missing package-SCC equalization step had left peer packages at different levels.
This finding was only detectable through automated structural analysis; the
checker's own 155-test suite was fully green throughout.
