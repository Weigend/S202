...
\title{Verifiable Layered Architecture Visualizations
       ...
       \\ \normalsize\textit{[Draft Version 3 -- for co-author review]}}}
...
a regular dependency may point against the intended layer direction, or a
top-level wiring class may visually sink below the sub-package it orchestrates.
Layout correctness should be the tool's responsibility, not the reader's---this
...
\par\smallskip
Correct level computation requires two separations. First, the architectural
analysis must distinguish class-level dependency depth from package-level
dependency depth: both are computed as the longest path in the respective
SCC-collapsed dependency graph (unweighted for classes, weighted by method-call
counts for packages), and neither is derived from the other. Second, and
independently of both, the visual position of an element inside its parent
container is governed by a per-parent local layer index, computed on the
sibling-only dependency graph closed within that parent's subtree. The local
layer index is what places \emph{ArchitectureView} above the \emph{rendering}
sub-package it calls into, without disturbing the global dependency depths.
Highly cyclic code bases are handled by a weight-based rank heuristic that
classifies back edges explicitly rather than hiding them.
\par\smallskip
The Layout Invariant Checker verifies five postconditions. Two are
implementation sanity checks on the global architecture levels: non-heuristic
class dependencies must respect class-level direction (R1-algo), and the
dominant direction in the weighted package graph must be reflected in package
levels (R3). Two are layout-level statements that the rendered picture is
honest: every class dependency must point downward in the visual rank derived
from the local layer index along the parent chain (R1-visual), and packages
forming a cyclic group in the filtered package dependency graph must share an
architecture level (R2). Finally, R5 keeps cached edge classifications in sync
with the current level and SCC state. The checker distinguishes implementation
bugs from tolerated heuristic violations and emits a copy-paste-ready
reproducer block for every finding.
\par\smallskip
...
equalization step and directly triggered a pipeline fix. On Minecraft
Forge~1.19.2, separating analysis depth from local layout drops the visual
upward-edge count from roughly~6\,000 to~3\,600 by eliminating the artefacts
of a previous parent-filtered weight pipeline; the remaining edges are real
architectural debt. No previous layered architecture visualization known to
us has provided this degree of machine-checkable self-validation.}
%% ---------------------------------------------------------------
...
\begin{center}
  \fbox{\textbf{\large Draft Version 3 \quad|\quad \today}}
\end{center}
...
\subsection*{The Core Problem: Two Coordinate Worlds, Four Constraints}

A correct layered layout has to satisfy two coordinate concerns at once that
are easy to conflate but answer different questions:

\begin{itemize}
  \item \emph{Analysis (global)} -- where does this element sit in the
        dependency stack of the analysed system? This is the longest-path depth
        in the dependency graph and is best understood per element type:
        classes are levelled on the class graph, packages on the weighted
        inter-package graph. Both are global numbers.
  \item \emph{Layout (local)} -- where should the box for this element sit
        inside its parent container? This is decided by the dependency
        relationships among the parent's direct children only; refs that leave
        the parent are irrelevant to local position because the parent itself
        is positioned by the same logic one level up.
\end{itemize}

The two worlds are independent in principle and need to be kept independent in
the implementation. Mixing them is the root cause of every failure mode below.
Four invariants follow from the two-world separation:

\begin{enumerate}
  \item \textbf{Class-level analysis constraint (R1-algo):} If
        $A \rightarrow B$ denotes a class dependency, then
        $\text{archLvl}(A) > \text{archLvl}(B)$ after heuristic back-edges are
        removed. This is a sanity check on the longest-path assignment in the
        class-dependency SCC-DAG.
  \item \textbf{SCC constraint (R2):} Classes in a cyclic dependency group
        receive the same class architecture level; packages that form a cyclic
  ...
  \item \textbf{Visual-rank constraint (R1-visual):} For every class
        ...
        comparison is lexicographic on the tuple of local layer indices along
        the parent path, ending with the class's own local index. This is the
        statement the reader actually checks against the drawing.
\end{enumerate}

R1-algo and R3 guard the algorithm; R1-visual and R2 guard the picture. The
failure modes that follow show why each obvious shortcut collapses one of these
into the other and loses information.
...
\subsection*{The Solution: Three Coordinate Concepts on Independent Graphs}

Failure modes 1 to 3 share a root cause: conflating coordinate concepts that
belong to different semantic domains. S202 resolves this by computing three
independent coordinate values, two of them for analysis and one for layout.
...
\begin{itemize}
  \item \textbf{Class architecture level} is the longest path in the
        ...
        foundation of the analysed system.
  ...
  \item \textbf{Local layer index} is the longest path in the SCC-collapsed
        ...
        child of that parent; refs leaving the parent's subtree are excluded
        because they are the business of the enclosing container. Every
        element (class and package) gets a local layer index that is only
        ...
        local-layer-index world, not the global one.
\end{itemize}
...
hard to reason about, test, and maintain. Every obvious simplification of such
...
compute the global architecture levels; Phase~3 derives the local layer
index per parent container from the architecture-level output.
...
levels honestly: every class reference contributes to the weighted graph regardless of
...
\begin{algorithm}[H]
\DontPrintSemicolon
\caption{Phase~3 -- Local Layer Index per Parent Container (\texttt{LocalLayerCalculator}, Step~6)}
...
\Indp
  ...
  Within each SCC of size $\ge 2$: weight-based rank break (same formula as Phase~2).\;
  Condense to SCC-DAG, longest-path pass, assign \textbf{localLayerIndex} per sibling.\;
\Indm
\end{algorithm}

Phase~3 is a pure layout phase. It never modifies architecture levels. Each
parent container is a self-contained world: a class's local layer index inside
\emph{ui} carries no relationship to a class's local layer index inside
\emph{ui.rendering}. The renderer sorts each parent's children by their local
layer index, descending. Cross-container ordering is the business of the
enclosing parent's local layer index, applied recursively. Local cycles---two
sibling sub-packages with mutual class references---are resolved silently by
the rank break; they are not surfaced as a separate Tangle category because
the user-visible tangle list remains a global architecture statement.
...
\begin{tabular}{llll}
\hline
 & \textbf{Phase 1 -- Class} & \textbf{Phase 2 -- Package} & \textbf{Phase 3 -- Local Layer} \\
\hline
...
\textbf{Cycles}       & SCCBreaker (degree)          & Weight-rank, all edges        & Weight-rank within siblings \\
\textbf{Used by}      & R1-algo, Tangles, R2 (class) & R3, R2 (package)              & R1-visual, renderer sort \\
\hline
\end{tabular}
\caption{The three coordinate concepts computed by S202.}
\end{table}

\subsection*{Layout Invariants for Level Correctness}

The three level phases define five machine-checkable postconditions verified
after every analysis run. R1-algo, R2, R3, R5 are checked by the
\texttt{LayoutInvariantChecker} as algorithm sanity guards;
R1-visual is checked by the architecture-builder's violation detection on the
visual-rank tuple.
...
R1-visual  & For every non-heuristic class dependency $A \rightarrow B$, the
             rendered position of $A$ sits above $B$. Position is compared
             ...
             where $L$ denotes the local layer index. Covers same-parent,
...
R5         & Cached edge classifications (\texttt{NORMAL}, \texttt{VIOLATION},
             \texttt{BACK\_EDGE}) remain consistent with the current level
             assignment and SCC membership. \\
\hline
\end{tabular}
\caption{Five layout invariants verified after every analysis run. R1-algo,
R3 and R2 guard the algorithm; R1-visual guards the rendered picture; R5
keeps the cached annotations honest.}
\end{table}
...
immediate sanity check---a failing R1-algo signals a fundamental breakdown in
the class-level pipeline.

\textbf{R1-visual} (lexicographic comparison of local-layer-index tuples along
the parent chain) is the invariant the reader actually checks against the
drawing. The two checks are intentionally separated: R1-algo asks ``did Phase~1
compute consistent class architecture levels?'', R1-visual asks ``does the
rendered Y position honestly reflect the class dependencies?'' Both pass on a
correct implementation; only the second one is allowed to fail when the
sibling-only graph of some parent exposes a genuine upward edge, because
that is real architectural debt rather than an algorithm bug. The
...
shared parent context. No special-case logic is needed for parent$\to$child
...
is reflected in the architecture level assignment. For any package edge $(P_A, P_B)$ where
...
\Indp
  ...
  \hspace{1.5em} No child--parent lift: layout positioning is the local-layer-index world.\;
\Indm
\BlankLine
\textbf{Step~5:} Set reverse dependencies (dependents)\;
\BlankLine
\textbf{Step~6:} Compute local layer index per parent (Phase~3)\;
\Indp
  ...
  \hspace{1.5em} Tarjan $\to$ rank-based SCC break $\to$ longest-path;\;
  \hspace{1.5em} assign \texttt{localLayerIndex} to each direct child of $P$.\;
\Indm
\end{algorithm}
...
self-contained universe: only class refs that stay within the parent's subtree
...
\Indp
  \textbf{Step~1:} Group direct children by their \texttt{localLayerIndex} value\;
...
\Indm
\textbf{rendering:} sort a copied child view by \texttt{localLayerIndex} (Phase~3), then \texttt{horizontalLayoutOrder} (Phase~4), then full name\;
\end{algorithm}

Phase~4 does not change any level or layer assignment and is deliberately
...
keeping two coordinate worlds separate: \emph{analysis} (where does this
element sit in the global dependency stack?) and \emph{layout} (where should
its box go inside its parent container?). Every straightforward single-pass
approach over a unified graph collapses these two questions into one
coordinate, and the failure modes documented above show why each obvious
simplification breaks down in practice.

S202 resolves this by computing three coordinate values on three independent
...
\emph{Package architecture levels} are derived from a weighted inter-package
...
\emph{Local layer indices} are computed per parent container on the
...
overlays without modifying any coordinate or affecting any layout invariant.
The Layout Invariant Checker verifies five machine-checkable postconditions
(R1-algo, R1-visual, R2, R3, R5) after every analysis run and emits
copy-paste-ready reproducer blocks for each finding.

Two cases demonstrate the value of the separation. In \emph{software-ekg-7},
a plausible city layout reported 167 dependency violations and a maximum
...
the local layer index as a distinct layout coordinate and lifting the parent
filter from the weighted package graph reduced the visual upward-edge count
from roughly~6\,000 to~3\,600; the eliminated edges were artefacts of forcing
parent packages below their sub-packages to keep them out of the algorithm's
tangle list, and the remaining edges expose real architectural debt that
deserves a code-level fix.
...
\vspace{1em}
The companion class \texttt{LocalLayerCalculator} implements Phase~3 (Step~6).

\lstinputlisting[style=appendixjava]{../../analyzer/src/main/java/de/weigend/s202/domain/architecture/LocalLayerCalculator.java}
