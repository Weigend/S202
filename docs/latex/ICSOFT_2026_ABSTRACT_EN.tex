...
The separation of class levels and package levels is an engineering choice
driven by semantic clarity. A unified single-graph approach is theoretically
possible, but it conflates two semantically distinct concerns---global
architectural depth and module dependency order---into a single coordinate
system, making the pipeline hard to reason about, test, and maintain. More
concretely, every obvious simplification of such a unified approach leads to
...
Every straightforward single-pass approach over a unified graph fails to
satisfy all three correctly; the failure modes documented above show why
