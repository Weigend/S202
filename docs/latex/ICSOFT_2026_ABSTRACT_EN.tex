...
through vertical position: elements that depend on others sit higher,
foundational elements sit lower. Their value rests on a simple promise---an
upward edge should reveal an architectural anomaly, not a layout artifact. If
level computation is wrong, the drawing still looks plausible while misleading
the viewer.
\par\smallskip
Levelized dependency maps are not new. Structure101's Levelized Structure Map
arranges elements into rows and exposes feedback in cyclic regions. Published
...
\par\smallskip
In this paper, we present the level pipeline implemented in S202, an
open-source Java architecture-visualization tool under Apache~2.0. The pipeline has three stages
on three independent graphs. First, S202 detects global SCCs in the class
graph and heuristically cuts large cycles. Second, from weighted package
dependencies it derives an architectural hypothesis: more heavily used packages
sit lower, their users higher. Third, it computes a local level inside
each parent container, which determines the vertical placement of siblings.
\par\smallskip
...
consistent: class dependencies respect class levels after heuristic cuts, dominant
...
cached edge classifications match the current SCC and level state. The checks
distinguish pipeline defects from deliberately tolerated heuristic cuts and
from architecture violations that the visualization is meant to expose.
\par\smallskip
...
architecture violations or explicit heuristic cuts. The result is a corrected, reproducible
level-computation algorithm whose visible anomalies can be explained and
checked.}
%% ---------------------------------------------------------------
...
This supplementary material expands the one-page abstract. The level
pipeline in S202 computes three independent ordering values, each on its
own graph: a \emph{class order} (SCC-collapsed class dependency DAG), a
\emph{package order} (weighted inter-package dependency graph), and a
\emph{local level} per parent container. Only the local level drives
the rendered layout; class order feeds the tangle view and the R1-algo
consistency check, package order feeds R2 and R3 and acts as a hard
constraint on the local level computation. The algorithmic contribution
is that no phase may rewrite the output of an earlier phase --- Phase 3
may not reorder packages relative to the order Phase 2 has already
computed.
...
\begin{figure}[H]
  ...
  \caption{The S202 ArchitectureView with class and package boxes.
    ...
    (\texttt{com.example.\ldots}). \textbf{Right:} a real-world cycle
    between \texttt{core}, \texttt{infra} and \texttt{service}.}
  \label{fig:s202-deps-overview}
\end{figure}

Figure~\ref{fig:s202-deps-overview} shows the two layout regimes the
pipeline has to handle. Left: an acyclic test project, where every
dependency points downward and the architecture reads at a glance.
Right: a real-world cycle between \texttt{core}, \texttt{infra} and
\texttt{service}, where the remaining upward edges are exactly the
architecturally interesting findings that the invariants surface
rather than hide.

The appendix is organised in three parts:
Appendix~A documents the failure modes that motivate the
three-coordinate model;
Appendix~B gives the complete reference implementation;
Appendix~C collects answers to recurring co-author questions.
