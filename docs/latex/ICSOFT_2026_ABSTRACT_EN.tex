...
through vertical position: elements that depend on others are placed higher.
Their practical value depends on a simple promise: upward edges should denote
real architectural feedback or deliberately exposed heuristic cuts, not artifacts
of the layout algorithm. If level computation is wrong, the drawing can still
look plausible while misleading the viewer. The goal is therefore not another
rendering metaphor, but a level-computation pipeline whose ordering decisions
are explicit, reproducible, and internally checked.
\par\smallskip
Levelized dependency maps are not new. Software-city layouts use
dependency-derived ground positions for class buildings~\cite{Dashuber2021},
and Structure101's Levelized Structure Map arranges software elements into rows
while exposing feedback dependencies in cyclic regions~\cite{HeadwayLSM,Muccini2012,HeadwayMatrix}. However, the published descriptions mainly specify
the intended visual behaviour; they do not make the concrete level-computation
algorithm and its consistency conditions reproducible. As a result, it remains
difficult to tell whether an upward edge in a generated picture is a genuine
architecture finding, a tolerated cycle-breaking artifact, or an implementation
defect.
\par\smallskip
This paper reports the level pipeline implemented in S202, an open-source Java
architecture-visualization tool under Apache~2.0. The central idea is to
separate three questions that are often treated as one ordering problem. A class architecture level
is the longest path in the SCC-collapsed class dependency graph. A package
architecture level is computed independently on a weighted inter-package graph,
where package order follows the dominant direction of aggregated method-call
traffic rather than the deepest contained class. A local layer index is computed
per parent package on the sibling-only dependency graph and determines the
visible order of classes and sub-packages inside that container. This separation
avoids feedback loops in which class levels lift packages, package movement
changes local order, and sorting no longer has a stable input graph. Highly
cyclic code bases are handled by a weight-based SCC-breaking heuristic whose cut
edges are classified explicitly instead of being hidden.
\par\smallskip
The three-level separation is the algorithmic contribution; invariant checks
provide machine-checkable evidence that a run is internally consistent. They
check, for example, that non-heuristic class dependencies respect class levels,
dominant weighted package dependencies respect package levels, cyclic peer
packages share a level after filtering, visual ranks follow local layer indices,
and cached edge classifications match the current SCC and level state. The
checks distinguish pipeline defects from architecture violations that the
visualization should expose.
\par\smallskip
Two real systems provide technical evidence for the approach. In the
92-module \emph{software-ekg-7} system, an invariant finding exposed a missing
package-SCC equalization step and triggered a pipeline fix. On Minecraft
Forge~1.19.2, separating global architecture depth from local layout reduces the
visual upward-edge count from roughly~6\,000 to~3\,600 by removing artifacts of
a previous parent-filtered weight pipeline; the remaining upward edges are
classified as architecture violations or explicit heuristic cuts. The result is
a corrected, reproducible level-computation algorithm for layered architecture
visualization, supported by consistency checks rather than by informal visual
inspection.}
%% ---------------------------------------------------------------
...
\subsection*{The Core Problem: Three Computed Values, Five Consistency Checks}

A correct layered layout has to answer three related questions that are often
treated as one ordering problem:
...
\begin{itemize}
  ...
  \item \emph{Package analysis} -- where does a package sit in the global
        package dependency order? This is computed on the weighted
        inter-package graph, independently of the deepest contained class.
  \item \emph{Local layout} -- where should an element's box sit inside its
        parent container? Only dependencies among that parent's direct
        children decide this local layer index.
\end{itemize}

The three computed values are independent in principle, and the implementation
must keep them independent. Mixing them is the root cause of every failure mode
below. Five consistency checks then inspect the resulting levels and rendered
order:
...
The three level phases are guarded by five machine-checkable consistency
checks. The \texttt{LayoutInvariantChecker} checks R1-algo, R2, R3, and R5 as
algorithm consistency checks after every analysis run; the architecture builder
checks R1-visual through violation detection on the visual-rank tuple. These
checks are evidence that the computed model is internally consistent; they are
not the primary algorithmic contribution.
...
Specifying invariants for a three-value level pipeline turns out to be
...
Five machine-checkable consistency checks (R1-algo, R1-visual, R2, R3, R5) run
after each analysis and emit reproducer blocks for each finding. They provide
regression protection and diagnostic evidence; the algorithmic contribution is
the separation of the three values themselves.
