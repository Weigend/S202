...
\par\smallskip
Outside the software-city line of work, a closely related practical predecessor
is Structure101's Levelized Structure Map (LSM)~\cite{HeadwayLSM,Muccini2012}.
LSMs also arrange software elements into dependency-derived rows and expose
feedback dependencies in cyclic regions. The public documentation specifies
this intended behaviour---including that cyclic regions are arranged so that
only feedback dependencies flow upward~\cite{HeadwayMatrix}---but, to the best
of our knowledge, does not publish the concrete level-computation algorithm.
Our contribution is therefore not the visual idea of a levelized dependency
map, but an explicit, reproducible algorithmic pipeline for computing such
levels.
\par\smallskip
...
dependency graph. Second, S202 computes a package architecture level on a weighted
inter-package graph: package order follows the dominant direction of aggregated
method-call traffic, not the deepest class contained in the package. Third,
S202 computes a local layer index per parent package on the sibling-only dependency
...
stable input graph. S202 handles highly cyclic code bases with a weight-based SCC
...
\begin{itemize}
  \item \emph{Analysis (global)} -- where does this element sit in the
        ...
        the algorithm levels classes on the class graph and packages on the weighted
        inter-package graph. Both are global numbers.
  \item \emph{Layout (local)} -- where should the box for this element sit
        inside its parent container? Only the dependency
        relationships among the parent's direct children decide this; refs that leave
        the parent are irrelevant to local position because the algorithm applies
        the same logic to the parent one level up.
\end{itemize}

The two coordinate concepts are independent in principle, and the implementation must keep them independent.
Mixing them is the root cause of every failure mode below.
...
Outside the software-city line of work, Structure101's Levelized Structure Maps
are a closely related practical predecessor to the layout studied here. They
organize code elements into rows such that, as far as possible, dependencies
flow downward, and they expose feedback dependencies when cyclic regions
prevent strict levelization \cite{HeadwayLSM,HeadwayMatrix}. This makes
Structure101 an important point of comparison and a necessary part of the
related work.
...
visualization \cite{Muccini2012,Dashuber2022}: they characterize the LSM
concept, but do not provide an algorithmic specification of Structure101's
levelization procedure. To the best of our knowledge, no published paper
describes the exact Structure101 algorithm.
...
parent container gets its own computation: a class's local layer index inside
...
layer index, descending. The enclosing parent's local layer index handles
cross-container ordering recursively. The rank break resolves local cycles---two
sibling sub-packages with mutual class references---silently; S202 does not
surface them as a separate Tangle category because
...
The three level phases define five machine-checkable postconditions. The
\texttt{LayoutInvariantChecker} checks R1-algo, R2, R3, and R5 as algorithm
...
R1-visual  & For every non-heuristic class dependency $A \rightarrow B$, the
             rendered position of $A$ sits above $B$. The checker compares positions
...
\emph{Local layer indices} come from the per-parent
