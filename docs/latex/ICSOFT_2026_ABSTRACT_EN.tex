...
the rendered vertical layout; class order feeds the tangle view and
the class-level consistency check, package order feeds the
package-level consistency checks and acts as a hard constraint on
the local level computation. The algorithmic
contribution is that no phase may rewrite the output of an earlier
phase --- Phase 3 may not reorder packages relative to the order
Phase 2 has already computed.
...
\begin{itemize}\setlength{\itemsep}{2pt}
  \item \textbf{Phase 1 --- Class order.}
        ...
        Feeds the tangle view and the class-level consistency check;
        does not drive the layout.
  \item \textbf{Phase 2 --- Package order.}
        ...
        level}. Feeds the package-level consistency checks and is
...
The five invariants run \emph{after} the pipeline as consistency
checks; they are not a feedback signal that drives further iteration.
Every visible upward edge in the resulting drawing is therefore one
of three things: an architecture violation, an explicit heuristic
cut, or a peer-level cycle that could not be broken without
contradicting the package architecture --- never an accidental
...
After Phase~3 the level pipeline is finished. A fourth phase, the
