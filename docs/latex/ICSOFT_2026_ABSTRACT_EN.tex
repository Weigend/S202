...
\paragraph{Where the arrows come from.} The arrows in the view are not
produced by the four pipeline phases. They are the class-to-class
dependencies extracted from the bytecode in the very first analysis
step, long before any level is computed. Phases~1--3 decide \emph{where
the boxes go}; the renderer then simply draws an arrow from each source
box to each target box. Every arrow falls into one of four classes,
assigned by the pipeline before rendering:
\begin{itemize}\setlength{\itemsep}{1pt}
  \item \texttt{NORMAL} -- source box ends up above target box (downward arrow);
  \item \texttt{VIOLATION} -- source box ends up below target box even though
        the edge was not cut (upward arrow, drawn red);
  \item \texttt{INTRA\_SCC} -- both endpoints sit in the same cyclic group
        (sideways arrow within a peer level);
  \item \texttt{HEURISTIC\_CUT} -- the edge was explicitly removed by the
        SCC-breaker to obtain a usable hierarchy (also drawn red, but as a
        deliberate cut, not a finding).
\end{itemize}
...
drawing is therefore one of three things: an architecture violation
(\texttt{VIOLATION}, raising an R1-visual alert), an explicit
\texttt{HEURISTIC\_CUT}, or an \texttt{INTRA\_SCC} edge that could not be
broken without contradicting the package architecture --- never an
accidental artifact of the layout algorithm.
...
\begin{figure}[H]
  ...
  \caption{The S202 ArchitectureView with class and package boxes.
    ...
    (\texttt{com.example.\ldots}) --- no upward edges, no alerts.
...
architecturally interesting findings that the R1-visual alert surfaces
rather than hides.
...
\begin{figure}[H]
  ...
  \caption{Implausibility alert in action. For this screenshot we deliberately
    ...
    consistency checks that run after every analysis.}
...
S202 also continues a line of work the authors have published on layered
